    ...
    def _velocity(self,a): # {{{
        ...
        if abs(dx) < 60:
            if self.sim.getAgentPosition(a['id'])[1] < 300:
                floor = 8
            elif self.sim.getAgentPosition(a['id'])[1] > 340 and self.sim.getAgentPosition(a['id'])[1] < 640:
                floor = 7
            elif self.sim.getAgentPosition(a['id'])[1] > 680 and self.sim.getAgentPosition(a['id'])[1] < 980:
                floor = 6
            elif self.sim.getAgentPosition(a['id'])[1] > 1020 and self.sim.getAgentPosition(a['id'])[1] < 1320:
                floor = 5
            elif self.sim.getAgentPosition(a['id'])[1] > 1360 and self.sim.getAgentPosition(a['id'])[1] < 1660:
                floor = 4
            elif self.sim.getAgentPosition(a['id'])[1] > 1700 and self.sim.getAgentPosition(a['id'])[1] < 2000:
                floor = 3
            elif self.sim.getAgentPosition(a['id'])[1] > 2040 and self.sim.getAgentPosition(a['id'])[1] < 2340:
                floor = 2
            elif self.sim.getAgentPosition(a['id'])[1] > 2380 and self.sim.getAgentPosition(a['id'])[1] < 2680:
                floor = 1
            else:
                floor = 0
            try:
...
#liczba oczekujących na danym piętrze
    ...
    def _run(self):# {{{
        ...
        #plt.legend([red, blue], ["Prędkość w klatce schodowej","Prędkość z oczekującymi agentami"], loc="center right")
        ###for a,b in zip(x,y):
        ##    #print(a,b)
        ###plt.scatter(x, y)
        #plt.xlabel('Czas [krok]')
        #plt.ylabel('Prędkość [%]')
        ##plt.savefig("/home/mateusz/Pulpit/praca/dwie/predkosc_czas1.png")
        #plt.show()

        #ob, = plt.plot(xx,y, "o")
        #plt.xlabel('Gęstość [%]')
        #plt.ylabel('Prędkość [%]')
        ##plt.savefig("/home/mateusz/Pulpit/praca/dwie/gestosc_predkosc1.png")
        #plt.show()
        #plt.plot(donex, done, "o", color="red")
        #plt.xlabel('Czas [krok]')
        #plt.ylabel('Liczba agentów w klatce schodowej')
        #plt.savefig("/home/mateusz/Pulpit/liczba_czas1.png")
        #plt.show()
        #plt.plot(donex, done2, "o", color="red")
        #plt.xlabel('Czas [krok]')
        #plt.ylabel('Liczba ewakuowanych agentów')
        #plt.savefig("/home/mateusz/Pulpit/liczbaopuszczonych_czas1.png")
        #plt.show()
